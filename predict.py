...
def parse_args():
    ...
    parser.add_argument('--filepath', dest='filepath', default='flowers/test/100/image_07899.jpg') # deafault filepath to a blanket flower image 
...
def predict(image_path, model, topk=3, device = 'device'):
...
def main(): 
    ...
    probs, classes = predict(img_path, model, int(args.top_k), device)
    ...
    i=0 # Prints out top k classes and probs as according to user 
    while i < len(labels):
